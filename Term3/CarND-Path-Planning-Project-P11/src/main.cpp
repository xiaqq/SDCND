					...
					if (s != "") {
						...
						if (event == "telemetry") {
...

							{
								...
								if(d<(2+4*lane+2) && d >(2+4*lane-2))// the cars in the same lane
								{
							...
							if(too_close)
							{
								...
								if(lane>0)
								{
									...
									else if(lane<2 && (not right_occupied))
									{
							...
							double target_y = s(target_x); //spline
							...
							for (int i=1; i<=50-previous_path_x.size();i++) {
								...
								double N = (target_dist/(0.02 * ref_vel/2.24)); //mph km conversion
