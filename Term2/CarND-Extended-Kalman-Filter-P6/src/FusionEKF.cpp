...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
	...
	if (!is_initialized_) {
		...
		if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
			...
			ekf_.x_(2) = 0; //couldn't use phi, since this phi is not the angle between the dot_rho and vx
			ekf_.x_(3) = 0;
	...
	if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
		...
		cout << "update H as Hj" << endl;
		Hj_ = tool.CalculateJacobian(ekf_.x_); //Jakobie Matrix using predicted x state vector!!!
